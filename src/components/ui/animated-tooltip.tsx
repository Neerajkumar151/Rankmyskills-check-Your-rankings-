...
export const AnimatedTooltip = ({
    ...
    const handleMouseMove = (event: React.MouseEvent<HTMLImageElement, MouseEvent>) => {
        // getBoundingClientRect is safer across browsers than offsetWidth on the target
            ...
            {items.map((item) => (
                ...
                >
                    ...
                    <img
                        ...
                        src={item.image}
                        alt={item.name}
                        className="object-cover !m-0 !p-0 object-top rounded-full h-10 w-10 sm:h-12 sm:w-12 border-2 group-hover:scale-105 group-hover:z-30 border-white dark:border-zinc-900 relative transition duration-500"
                    />
                </div>
            ))}
        </div>
